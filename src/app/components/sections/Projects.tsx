          ...
          <div className="space-y-4">
            {projects.map((p) => (
              <motion.article
                key={p.id}
                variants={fadeUp}
                className="group rounded-2xl border border-border bg-card overflow-hidden hover:border-accent/25 transition-all duration-300 cursor-pointer"
                onClick={() => setExpanded(expanded === p.id ? null : p.id)}
              >
                <div className="grid lg:grid-cols-[380px_1fr] gap-0">
                  <div className="relative h-52 lg:h-56 overflow-hidden bg-muted shrink-0">
                    <img
                      src={p.image}
                      alt={p.name}
                      loading="lazy"
                      className="w-full h-full object-cover opacity-60 group-hover:opacity-80 group-hover:scale-[1.03] transition-all duration-600"
                    />
                    <div className="absolute inset-0 bg-gradient-to-r from-transparent via-transparent to-card hidden lg:block" />
                    <div className="absolute inset-0 bg-gradient-to-t from-card via-card/30 to-transparent lg:hidden" />
                  </div>
                  ...
                  <div className="p-8 lg:p-10 flex flex-col justify-center">
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        ...
                        <h3 className="font-serif text-2xl lg:text-3xl font-bold text-foreground mt-1">
                          {p.name}
                        </h3>
                      </div>
                      <ChevronRight
                    ...
                    <p className="text-muted-foreground text-sm leading-[1.7] mb-5">
                      {p.tagline}
                    </p>
                ...
                <AnimatePresence>
                  {expanded === p.id && (
                    ...
                    >
                      <div className="p-8 lg:p-10 grid sm:grid-cols-3 gap-8 lg:gap-12">
                        <div>
